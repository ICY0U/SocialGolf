...
ASGBasketball::ASGBasketball()
{
    ...
    if (SphereMeshAsset.Succeeded())
    {
        ...
        // FIXED: Scale to match new collision sphere size
        BallMesh->SetRelativeScale3D(FVector(BallRadius / 50.0f)); // Scale to match collision sphere
...
void ASGBasketball::Tick(float DeltaTime)
{
    ...
    if (bHasBeenThrown && IsValid(LastThrower))
    {
        ...
        if (CollisionComponent && CollisionComponent->GetPhysicsLinearVelocity().Size() < 50.0f)
        {
            RegisterMissedShot();
        }
    }
...
void ASGBasketball::SetupPhysics()
{
    if (CollisionComponent)
    {
        ...
        // Enhanced damping for more realistic basketball movement
        CollisionComponent->SetLinearDamping(0.02f); // Very low linear damping for better flight
        CollisionComponent->SetAngularDamping(0.08f); // Moderate angular damping for realistic spin
...
bool ASGBasketball::CanBePickedUp() const
{
    ...
    if (bHasBeenThrown)
    {
        ...
        if (CollisionComponent)
        {
            ...
            // Allow pickup if velocity is low (ball has settled) or 3 seconds have passed
            bool bHasSettled = CurrentVelocity < 100.0f || TimeSinceThrow > 3.0f;
